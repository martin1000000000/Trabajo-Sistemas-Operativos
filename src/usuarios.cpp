...
void guardarUsuariosEnArchivo(ListaUsuarios& listaUsuarios, const string& archivoUsuarios) {
    ...
    sort(listaUsuarios.usuarios, listaUsuarios.usuarios + listaUsuarios.cantidad, [](const Usuario& a, const Usuario& b) {
        return a.id < b.id;
    });
...
void ingresarUsuario(ListaUsuarios& listaUsuarios, const string& archivoUsuarios, const ListaPerfiles& listaPerfiles) {
    ...
    // Verificar que el ID no exista ni sea menor o igual a 0
    for (int i = 0; i < listaUsuarios.cantidad; i++) {
